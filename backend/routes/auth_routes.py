...
def auth_session(
  ...
  user = auth.get_session_user(pm_session, conn)
  if not user:
...
def auth_login(
  ...
  # secure=False: this MVP runs over plain HTTP on localhost.
  # For production, set secure=True to transmit the cookie only over HTTPS.
  response.set_cookie(
